    ...
    case "list":
        ...
        $i = 1;
            while ($reg = $query_Ingreso->fetch_object()) {
                $data[] = array(
                    "0"=>$i,
                    "1"=>utf8_encode($reg->proveedor),
                    "2"=>utf8_encode($reg->tipo_comprobante),
                    ...
                    "9"=>'<button class="btn btn-success" data-toggle="tooltip" title="Ver Detalle" onclick="cargarDataIngreso('.$reg->idingreso.',\''.$reg->serie_comprobante.'\',\''.$reg->num_comprobante.'\',\''.$reg->impuesto.'\',\''.$reg->total.'\',\''.$reg->idingreso.'\',\''.utf8_decode($reg->proveedor).'\',\''.$reg->tipo_comprobante.'\')" ><i class="fa fa-eye"></i> </button>&nbsp'.
